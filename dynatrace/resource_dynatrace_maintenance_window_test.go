...
func testAccCheckDynatraceMaintenanceWindowDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		mainteanceWindow, _, err := dynatraceConfigClientV1.MaintenanceWindowsApi.GetMaintenanceWindow(authConfigV1, mainteanceWindowID)
		if err == nil {
			if mainteanceWindow.Id == rs.Primary.ID {
...
func testAccCheckDynatraceMaintenanceWindowExists(n string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		_, _, err := dynatraceConfigClientV1.MaintenanceWindowsApi.GetMaintenanceWindow(authConfigV1, mainteanceWindowID)
		if err != nil {
			return err
		}
